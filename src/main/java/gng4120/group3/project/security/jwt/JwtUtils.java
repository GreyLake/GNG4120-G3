...
public class JwtUtils {
    ...
    @Value("${moomoney.app.jwtSecret}")
    private String jwtSecret;

    @Value("${moomoney.app.jwtExpirationMs}")
    private int jwtExpirationMs;

    @Value("${moomoney.app.jwtCookieName}")
    ...
    public ResponseCookie getCleanJwtCookie() {
        ResponseCookie cookie = ResponseCookie.from(jwtCookie, "").path("/").maxAge(0).build();
        return cookie;
    }
